...
void A0::guiLogic()
{
	...
	ImGui::Begin("Shape Properties", &showDebugWindow, ImVec2(100,100), opacity,
		...
		// Change color of Green and Blue components using sliders
		ImGui::SliderFloat("Green Channel", &m_shape_color.g, 0.0f, 1.0f);
		ImGui::SliderFloat("Blue Channel", &m_shape_color.b, 0.0f, 1.0f);

		// Change rotation of shaped using slider. Can rotate from 0 to 2pi (one circle).
		ImGui::SliderFloat("Rotation Angle (rad)", &m_shape_rotation, 0.0f, 2*PI);
...
bool A0::keyInputEvent(int key, int action, int mods) {
	...
	if (action == GLFW_PRESS) {
		if (key == GLFW_KEY_EQUAL) {
			...
			// Increase shape size by a factor of 1.5.
			m_shape_size=m_shape_size*1.5;
		...
		if (key == GLFW_KEY_MINUS) {
			...
			// Decrease shape size by a factor of 1.5.
			m_shape_size=m_shape_size/1.5;
